...
<%
   ...
   /* LoginServlet loginServlet = new LoginServlet();
    Boolean judgeUpdate = loginServlet.isAdmin;*/
    ...
    List<String[]> data = JDBCTool.getDataFromMoneyDatabase(year,like);
%>
...
<div class="content">
    <div class="search-container">
        <form method="get" action="search_result_of_money" style="display: flex; width: 100%;">
            <input type="text" id="query" name="query" required placeholder="Enter anything you want to search in money page">
    ...
    <h2>
        Money Data for <%= (request.getParameter("year")==null ? "2018" : request.getParameter("year"))%>
    </h2>
    ...
    <table>
        ...
        <% if (isAdmin) { %>
        <a href="moneyAdd.jsp?year=<%= year %>" class="update-link">Add</a>
        <% } %>
        ...
        <tr>
            <th>English Name</th>
            <th>Chinese Name</th>
            <th>Assets in <%= (request.getParameter("year")==null ? "2018" : request.getParameter("year"))%> (billion US dollars)</th>
        ...
        <tr>
            ...
            <td>
                <% if (isAdmin) { %>
                <a href="update.jsp?year=<%= year %>&name=<%= row[0] %>&chineseName=<%= row[1] %>&money=<%= row[2] %>&pid=<%= row[3] %>" class="update-link">Update</a>
                <a href="javascript:void(0);" onclick="confirmDelete('<%=contextPath%>/money?type=del&year=<%= year %>&pid=<%= row[3] %>')" class="del-link">Delete</a>
                <script>
