...
<head>
    <meta charset="UTF-8">
    <title>Source Of Wealth Data</title>
    <style>
        body {
...
<div class="content">
    <div class="search-container">
        <form action="search_result_of_source" method="get" style="display: flex; width: 100%;">
            <input type="text" id="query" name="query" required placeholder="Enter anything you want to search in source of wealth page">
            <input type="submit" value="Search">
        </form>
    </div>
    <h2>Source Of Wealth Data for <%= (request.getParameter("year") == null ? "2018" : request.getParameter("year")) %></h2>
    ...
    <table>
        <% if (isAdmin) { %>
        <a href="sourceAdd.jsp?year=<%= (request.getParameter("year") == null ? "2018" : request.getParameter("year")) %>" class="update-link">Add</a>
        <% } %>
        <thead>
        <tr>
            <th>Source Of Wealth</th>
            <th>Year <%= (request.getParameter("year") == null ? "2018" : request.getParameter("year")) %></th>
        ...
        <%
            ...
            List<String[]> data = JDBCTool.getDataFromSourceDatabase(year, like);
        ...
        <tr>
            ...
            <td>
                <% if (isAdmin) { %>
                <a href="sourceUpdate.jsp?year=<%= year %>&source=<%= row[0] %>&money=<%= row[1] %>" class="update-link">Update</a>
                <a href="javascript:void(0);" onclick="confirmDelete('<%=contextPath%>/source?type=del&year=<%= year %>&source=<%= row[0] %>')" style="color:red;">Delete</a>
                <script>
