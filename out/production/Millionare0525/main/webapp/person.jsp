...
<div class="content">
    <div class="search-container">
        <form method="get" action="search_result_of_person" style="display: flex; width: 100%;">
            <input type="text" id="query" name="query" required placeholder="Enter anything you want to search in person page">
    ...
    <table>
        ...
        <% if (isAdmin) { %>
        <a href="personAdd.jsp" class="update-link">Add</a>
        <% } %>

        <%
            String like = request.getParameter("like");

            List<String[]> data = JDBCTool.displayAllDataFromPersonTable(like);
            ...
            char currentInitial = ' ';
            for (String[] row : data) {
                char initial = row[1].toUpperCase().charAt(0);
        ...
        <tr>
            ...
            <td>
                <% if (isAdmin) { %>
                <a href="updateBypeople.jsp?year=<%= year %>&pid=<%= row[0] %>&name=<%= row[1] %>&chineseName=<%= row[2] %>&country=<%= row[3] %>&food=<%= row[4] %>&source=<%= row[5] %>" class="update-link">Update</a>
                <a href="javascript:void(0);" onclick="confirmDelete('<%=contextPath%>/person?type=del&pid=<%= row[0] %>')" class="del-link">Delete</a>
                <script>
